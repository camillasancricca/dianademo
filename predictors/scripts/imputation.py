...
categorical_variables = ['cp','thal']
numerical_variables = ['oldpeak']
...
class impute_knn():
    ...
    def fit(self, df, missing_column, n_neighbors=5):
        ...
        if type_missing in ["int64", "float64"]:
            ...
            X = utils.utils.encoding_categorical_variables(X)
            columns = X.columns
...
class impute_mice:
    ...
    def fit(self, df, missing_column, estimator):
        ...
        if type_missing in ["int64", "float64"]:
            ...
            X[numerical_variables] = X[numerical_variables].fillna(-1)

            X = pd.DataFrame(imputer.fit_transform(X), columns=columns)

            X[numerical_variables] = X[numerical_variables].replace(-1, np.nan)
...
def impute(df, method, missing_column):
    ...
    if method == "no_impute":
        ...
        imputated_df = imputator.fit(df, missing_column)
        if len(imputated_df.columns) > 3:
            if missing_column == categorical_variables[0]:
                imputated_df = utils.decode_categorical_variables(imputated_df, categorical_variables[1])
            elif missing_column == categorical_variables[1]:
                imputated_df = utils.decode_categorical_variables(imputated_df, categorical_variables[0])
            else:
                imputated_df = utils.decode_categorical_variables(imputated_df, categorical_variables[0])
                imputated_df = utils.decode_categorical_variables(imputated_df, categorical_variables[1])

...
from explainability.explainations import get_explanation_summary, get_explanation_alerts, get_explanation_correlations, get_explanation_missing, get_explanation_attribute

st.set_page_config(page_title="Profiling", layout="wide", initial_sidebar_state="expanded")
...
# Creazione della tabella in Streamlit con sfondo bianco
    ...
    with st.expander("More Details on Data Summary"):
        exp1, exp2 = get_explanation_summary(df)
        st.write(exp1)
        st.write(exp2)

    with st.expander("Possible Data Quality Issues"):
        exp1, exp2 = get_explanation_alerts(df)
        st.write(exp1)
        st.write(exp2)
    ...
    with st.expander("More Details on Correlations Analysis"):
        exp1, exp2 = get_explanation_correlations(df)
        st.write(exp1)
        st.write(exp2)
...
# Filtra i valori NaN e infiniti per la colonna selezionata
    ...
    with st.expander("More Details on Column "+selected_column):
        exp1, exp2 = get_explanation_attribute(df, selected_column)
        st.write(exp1)
        st.write(exp2)
...
# Seleziona una colonna dalla lista tramite selectbox
    ...
    with st.expander("More Details on Column "+selected_col):
        exp1, exp2 = get_explanation_attribute(df, selected_col)
        st.write(exp1)
        st.write(exp2)
...
# Mostra il grafico nella pagina Streamlit
    ...
    with st.expander("More Details on Missing Values"):
        exp1, exp2 = get_explanation_missing(df)
        st.write(exp1)
        st.write(exp2)
